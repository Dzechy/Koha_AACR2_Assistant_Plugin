...
sub _format_provider_error {
    ...
    if ($status_code == 429) {
        $message .= ' Rate limit reached. Wait 30-60 seconds, reduce request frequency/retries, or use a model/account with more quota.';
    }
    return $message;
}
...
sub _extract_response_text {
    ...
    if ($response->{choices} && ref $response->{choices} eq 'ARRAY') {
        for my $choice (@{ $response->{choices} }) {
            ...
            if (defined $message->{content}) {
                my $content = $self->_extract_text_from_message_content($message->{content});
                return $content if defined $content && $content ne '';
            }
            my $delta = $choice->{delta} || {};
            if (defined $delta->{content}) {
                my $content = $self->_extract_text_from_message_content($delta->{content});
                return $content if defined $content && $content ne '';
            }
        }
    }
    if ($response->{message} && ref $response->{message} eq 'HASH') {
        if (defined $response->{message}{content}) {
            my $content = $self->_extract_text_from_message_content($response->{message}{content});
            return $content if defined $content && $content ne '';
        }
    }
    my $content = '';
