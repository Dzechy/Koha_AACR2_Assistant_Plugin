...
sub run {
    ...
    unless ( defined $plugin_class && length $plugin_class ) {
        warn "Koha::Plugins::Handler->run called without plugin class";
        return;
    }

    unless ( defined $plugin_method && length $plugin_method ) {
        warn "Koha::Plugins::Handler->run called without plugin method";
        return;
    }

    # Only allow normal Perl method identifiers
    unless ( $plugin_method =~ /\A[A-Za-z_]\w*\z/ ) {
        warn "Invalid plugin method name '$plugin_method' for class '$plugin_class'";
        return;
    }
    ...
    if ($has_method) {
        ...
        my $loaded = eval { load $plugin_class; 1 };
        unless ($loaded) {
            my $err = $@ || 'Unknown error';
            warn "Failed to load plugin class '$plugin_class': $err";
            return;
        }

        my $plugin = eval { $plugin_class->new( { cgi => $cgi, enable_plugins => $args->{'enable_plugins'} } ) };
        if ( my $err = $@ ) {
            warn "Failed to instantiate plugin '$plugin_class': $err";
            return;
        }

        unless ( $plugin && $plugin->can($plugin_method) ) {
            warn "Plugin '$plugin_class' is registered for '$plugin_method' but method is not callable";
            return;
        }

        my $rv = eval { $plugin->$plugin_method($params) };
        if ( my $err = $@ ) {
            warn "Error running plugin '$plugin_class' method '$plugin_method': $err";
            return;
        }
        ...
        # Avoid undef concatenation warnings
